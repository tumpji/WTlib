...
namespace Predictor {
    void Up::forward_predict(Shared_data data) {
        ...
        for( int radek = rozmer_y - 1; radek > 0; --radek )
        {
            ...
            for ( int sloupec = rozmer_x - 1; sloupec >= 0; --sloupec, --index ) {
                auto& A = raw_data[index-rozmer_x];
                int predikce = A;
    ...
    void Up::reverse_predict(Shared_data data) {
        ...
        while( index < rozmer_x*rozmer_y )
        {
            ...
            // vsechny ostatni
            for ( int sloupec = 0; sloupec < rozmer_x; ++sloupec, ++index )
            {
                auto& A = raw_data[index-rozmer_x];
                int predikce = A;
